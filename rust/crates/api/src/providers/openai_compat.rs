...
impl OpenAiCompatClient {
    ...
    /// Like [`from_env`] but treats a missing API key as an empty string.
    /// Used for providers like Ollama that don't require authentication.
    pub fn from_env_with_optional_key(config: OpenAiCompatConfig) -> Result<Self, ApiError> {
        let api_key = read_env_non_empty(config.api_key_env)?.unwrap_or_default();
        Ok(Self::new(api_key, config))
    }
