...
fn strip_ollama_prefix(request: &MessageRequest) -> MessageRequest {
    let model = request
        .model
        .strip_prefix("ollama:")
        .or_else(|| request.model.strip_prefix("ollama/"))
...
impl ProviderClient {
    ...
    pub fn from_model_with_default_auth(
        ...
        match providers::detect_provider_kind(&resolved_model) {
            ...
            ProviderKind::Ollama => Ok(Self::Ollama(
                OpenAiCompatClient::from_env_with_optional_key(OpenAiCompatConfig::ollama())?,
            )),
        }
    }
    ...
    pub async fn send_message(
        ...
        match self {
            ...
            Self::Ollama(client) => {
                let stripped = strip_ollama_prefix(request);
                send_via_provider(client, &stripped).await
            }
        }
    }
    ...
    pub async fn stream_message(
        ...
        match self {
            ...
            Self::Ollama(client) => {
                let stripped = strip_ollama_prefix(request);
                stream_via_provider(client, &stripped)
                    .await
                    .map(MessageStream::OpenAiCompat)
            }
        }
