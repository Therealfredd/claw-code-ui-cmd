...
/// Prompts the user with a question showing a default value in brackets.
/// Returns the typed input, or `default` if the user presses Enter without typing.
/// Falls back to `default` when stdout is not a terminal.
pub fn prompt_confirm_path(prompt: &str, default: &str) -> io::Result<String> {
    ...
    // Use crossterm style commands so colors render via Windows Console API
    // (works in CMD without requiring VT/ANSI processing to be enabled).
    execute!(
        stdout,
        Print(format!("  {prompt} [")),
...
/// Displays an interactive list where the user can navigate with ↑/↓ or press a digit key,
/// then confirms with Enter (or cancels with Esc to keep `default_idx`).
/// Falls back to `default_idx` when stdout is not a terminal.
/// Accepts any pair of values that implement `AsRef<str>`, so both `&str` and `String` work.
pub fn select_from_list<S: AsRef<str>>(
    title: &str,
    items: &[(S, S)],
    default_idx: usize,
) -> io::Result<usize> {
    if !io::stdout().is_terminal() || items.is_empty() {
        return Ok(default_idx.min(items.len().saturating_sub(1)));
    }

    let _raw = RawModeGuard::new()?;
    let mut stdout = io::stdout();
    let mut selected = default_idx.min(items.len().saturating_sub(1));
    ...
    loop {
        ...
        if rendered_lines > 0 {
            queue!(
                stdout,
                MoveUp(to_u16(rendered_lines - 1)?),
                MoveToColumn(0),
        ...
        // Render items
        for (idx, (name, desc)) in items.iter().enumerate() {
            let name = name.as_ref();
            let desc = desc.as_ref();
            if idx == selected {
                write!(
                    stdout,
                    "  \x1b[1;38;5;45m\u{2192} {:<3} {:<36} {}\x1b[0m\r\n",
                    idx + 1,
                    name,
                    desc
                )?;
        ...
        // Render hint (no trailing \r\n so cursor stays on this line)
        write!(
            stdout,
            "\r\n  \x1b[2m\u{2191}/\u{2193} navigate  1-9 jump  Enter confirm  Esc default\x1b[0m"
        )?;
        rendered_lines += 2;
        stdout.flush()?;
