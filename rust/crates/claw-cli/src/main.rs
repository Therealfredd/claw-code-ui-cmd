...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::Serve { port, model } => run_serve(port, model)?,
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    match rest[0].as_str() {
        ...
        "serve" | "ui" => parse_serve_args(&rest[1..], &model),
        "prompt" => {
...
fn parse_serve_args(args: &[String], default_model: &str) -> Result<CliAction, String> {
    ...
    Ok(CliAction::Serve { port, model })
}

fn run_serve(port: u16, model: String) -> Result<(), Box<dyn std::error::Error>> {
    // Search for web/dist in several candidate locations:
    //   1. Next to the release binary (installed / cargo install)
    //   2. CWD/web/dist       (running from project root)
    //   3. CWD/../web/dist    (running from rust/ subdirectory via cargo run)
    let web_dist = std::env::current_exe()
        .ok()
        .and_then(|exe| exe.parent().map(|p| p.join("../web/dist")))
        .filter(|p| p.exists())
        .or_else(|| {
            let cwd = std::env::current_dir().ok()?;
            for rel in &["web/dist", "../web/dist"] {
    ...
    let mut state = server::AppState::new().with_default_model(model);
    if let Some(dir) = web_dist {
        state = state.with_static_dir(dir.canonicalize().unwrap_or(dir));
    }

    let addr = format!("127.0.0.1:{port}");
    println!("Claw UI listening on http://{addr}");
    if let Err(error) = open_browser(&format!("http://{addr}")) {
        eprintln!("Note: could not open browser automatically ({error})");
    }

    let runtime = tokio::runtime::Runtime::new()?;
    runtime.block_on(server::listen_and_serve(&addr, state))?;
    Ok(())
}
...
/// Interactive startup wizard that confirms the workspace directory and lets the user
/// choose a model from a numbered list. Returns the selected model name.
/// Skips interactive prompts when stdout is not a terminal.
fn run_startup_wizard(
    ...
    // Header — use crossterm style so Windows Console API renders colors even
    // without VT/ANSI processing enabled in the terminal.
    if color {
        execute!(
            stdout,
            SetAttribute(Attribute::Bold),
            SetForegroundColor(Color::Cyan),
            Print("\u{1F99E} Claw Code"),
            ResetColor,
            SetAttribute(Attribute::Dim),
            Print(" \u{2014} Startup"),
            ResetColor,
            Print("\n\n"),
        )?;
    } else {
        println!("Claw Code \u{2014} Startup\n");
    }
    ...
    let confirmed = input::prompt_confirm_path("Workspace", &cwd_str)?;
    if confirmed != cwd_str {
        let new_path = PathBuf::from(&confirmed);
        env::set_current_dir(&new_path)
            .map_err(|e| format!("Cannot change to '{confirmed}': {e}"))?;
    }
    println!();

    // --- Step 2: model selection ---
    // Build the model list: static built-ins + any locally running Ollama models.
    // discover_ollama_models is async; spin up a minimal tokio runtime for the call.
    let ollama_names: Vec<String> = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map(|rt| rt.block_on(api::discover_ollama_models()))
        .unwrap_or_default();

    // Combine static models with discovered Ollama models into an owned list.
    let mut all_models: Vec<(String, String)> = AVAILABLE_MODELS
        .iter()
    ...
    for name in &ollama_names {
        all_models.push((
            format!("ollama:{name}"),
            format!("{name} \u{00B7} Ollama local"),
        ));
    }

    let default_idx = all_models
        .iter()
        .position(|(name, _)| name == preselected_model)
        .unwrap_or(0);
    ...
    let selected_idx = input::select_from_list("", &all_models, default_idx)?;
    let (selected_model, selected_desc) = &all_models[selected_idx];
    ...
    if color {
        execute!(
            ...
            ResetColor,
            SetAttribute(Attribute::Dim),
            Print(format!("  {selected_desc}")),
            ResetColor,
            Print("\n\n"),
        )?;
    } else {
        println!("  Model  {selected_model}  {selected_desc}\n");
    }
...
fn run_repl(
    ...
    let model = run_startup_wizard(&model)?;
...
impl LiveCli {
    ...
    /// Prints the startup banner to stdout using crossterm style commands so that
    /// colours render correctly on Windows CMD without requiring VT processing.
    fn print_startup_banner(&self) -> io::Result<()> {
        use crossterm::style::{Attribute, Color, Print, ResetColor, SetAttribute, SetForegroundColor};
        use crossterm::execute;
        ...
        let workspace_summary = git_branch.as_deref().map_or_else(
            ...
            |branch| format!("{workspace_name} \u{00B7} {branch}"),
        );
        let has_claw_md = cwd
            .as_ref()
        ...
        if color {
            ...
            writeln!(out, "Claw Code \u{00B7} ready")?;
        }
        ...
        let info: &[(&str, &str)] = &[
            ...
            (
                "Quick start",
                if has_claw_md {
                    "/help \u{00B7} /status \u{00B7} ask for a task"
                } else {
                    "/init \u{00B7} /help \u{00B7} /status"
                },
            ),
        ];
        ...
        if !has_claw_md {
            writeln!(
                ...
                "  {:<16} {}",
                "First run",
                "/init scaffolds CLAW.md, .claw.json, and local session files"
            )?;
        }
        ...
        if color {
            ...
            writeln!(out, "  Keyboard shortcuts")?;
        }
        for (keys, desc) in &[
        ...
        if color {
            execute!(
                ...
                ResetColor,
                SetAttribute(Attribute::Dim),
                Print(" (when /vim is enabled)"),
                ResetColor,
                Print("\n"),
            )?;
        } else {
            writeln!(out, "  Vim mode (when /vim is enabled)")?;
        }
        for (key, desc) in &[
            ("i", "Enter insert mode"),
        ...
        if color {
            execute!(
                ...
                ResetColor,
                SetAttribute(Attribute::Dim),
                Print(" (/help for full list)"),
                ResetColor,
                Print("\n"),
            )?;
        } else {
            writeln!(out, "  Slash commands (/help for full list)")?;
        }
        for (cmd, desc) in &[
...
impl DefaultRuntimeClient {
    fn new(
        ...
        // For Anthropic/Claw models load OAuth/API-key auth.
        // For all other providers (Ollama, xAI, OpenAI) credentials come from
        // environment variables or are optional (Ollama), so skip the Anthropic
        // credential check entirely.
        let client = if api::detect_provider_kind(&model) == ProviderKind::ClawApi {
